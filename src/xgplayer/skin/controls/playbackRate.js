...
let s_playbackRate = function () {
  ...
  let selectedSpeed = 1
  let tmp = ['<div class="xgplayer-control-item_popover"><ul>']
  list.forEach(item => {
  ...
  tmp.push(`</ul></div><p class='name'>${selectedSpeed}${unit}</p>`)
  ...
  ev.forEach(item => {
    container.addEventListener(item, e => {
      ...
      if(li && li.tagName.toLocaleLowerCase() === 'li') {
        ...
        li.parentNode.parentNode.nextSibling.innerHTML = `${li.getAttribute('cname')}${unit}`
