...
let s_definition = function () {
  ...
  function onCanplayResourceReady () {
    ...
    let tmp = ['<div class="xgplayer-control-item_popover"><ul>'], src = player.config.url, a = document.createElement('a')
    if (player.switchURL) {
    ...
    tmp.push(`</ul></div><p class='name'>${(cursrc[0] || {name: ''}).name}</p>`)
  ...
  ['touchend', 'click'].forEach(item => {
    container.addEventListener(item, function (e) {
      ...
      if (li && li.tagName.toLocaleLowerCase() === 'li') {
        ...
        li.parentNode.parentNode.nextSibling.innerHTML = `${li.getAttribute('cname')}`
